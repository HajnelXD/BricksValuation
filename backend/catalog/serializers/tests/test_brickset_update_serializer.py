...
class UpdateBrickSetSerializerTests(TestCase):
    ...
    def test_serializer_rejects_has_box_non_boolean(self) -> None:
        """Serializer rejects has_box with non-boolean value (DRF converts strings)."""
        # Note: DRF BooleanField coerces string values "yes"/"no" etc. to boolean
        # For true rejection, test with completely invalid types
        serializer = UpdateBrickSetSerializer(data={"has_box": 123})
